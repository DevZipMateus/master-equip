...
const Contact = () => {
  ...
  const contactInfo = [
    {
      icon: <Phone className="h-5 w-5 sm:h-6 sm:w-6 text-cyan-400" />,
      title: 'Telefone',
    ...
    {
      icon: <Mail className="h-5 w-5 sm:h-6 sm:w-6 text-cyan-400" />,
      title: 'Email',
    ...
    {
      icon: <MapPin className="h-5 w-5 sm:h-6 sm:w-6 text-cyan-400" />,
      title: 'Endereço',
    ...
    {
      icon: <Instagram className="h-5 w-5 sm:h-6 sm:w-6 text-cyan-400" />,
      title: 'Instagram',
  ...
  return (
    <section id="contato" ref={sectionRef} className="py-12 sm:py-16 lg:py-20 bg-slate-900">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
          ...
          >
            <div className="bg-slate-800/50 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg border border-slate-700">
              <div className="text-center mb-6 sm:mb-8">
                <div className="bg-green-600/20 w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                  <MessageCircle className="h-8 w-8 sm:h-10 sm:w-10 text-green-400" />
                </div>
          ...
          >
            <div className="space-y-4 sm:space-y-6">
              ...
              {contactInfo.map((item, index) => (
                <div key={index} className="bg-slate-800/50 rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-slate-700">
                  <div className="flex items-start space-x-3 sm:space-x-4">
                    <div className="bg-cyan-600/20 rounded-lg p-2 sm:p-3 flex-shrink-0">
                      {item.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-base sm:text-lg font-semibold text-white mb-1 sm:mb-2">{item.title}</h4>
                      <p className="text-sm sm:text-base text-gray-300 mb-2 sm:mb-3 break-words">{item.details}</p>
                      <a 
                        href={item.action} 
                        target={item.action.startsWith('http') ? '_blank' : '_self'}
                        rel={item.action.startsWith('http') ? 'noopener noreferrer' : ''}
                        className="inline-flex items-center text-xs sm:text-sm font-medium text-cyan-400 hover:text-cyan-300 hover:underline"
                      >
                        {item.actionText}
                      </a>
                    </div>
            ...
            <motion.div
              ...
              className="mt-6 sm:mt-8 bg-gradient-to-br from-slate-800 to-gray-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-white border border-slate-700"
            >
